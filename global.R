            ...
            if(!is.null(albums_list$df)) {
                  if(choose_artist_input$search_albums2 >0) {
                        ...
                        list(
                              ...
                              withBusyIndicatorUI(actionButton(ns("select_albums"), "Submit"))
                        )
                  }
            }
      })
      ...
      output$delete_UI <- renderUI({
            list(
                  selectizeInput(inputId = ns("artists_to_keep"), 
                                 label = "Choose which artists to select here", 
                                 choices = artists(),
                                 multiple = T,
                                 selected = artists(),
                                 options = list('plugins' = list('remove_button'))),
...
add_artistUI <- function(id) {
      ...
      tagList(
            box(title = "Choose an Artist Below",
                  artist_inputUI(ns("art")),
                  artist_chooseUI(ns("choo")),
                  br(),
                  artist_img_printUI(ns("img")),
...
add_artist_sidebarUI <- function(id) {
      ...
      tagList(
            ...
            tags$div(class = "footnote", tags$div("Created by Duncan Gates"), tags$div("duncan.gates123@gmail.com"), align = "center")
      )
}
