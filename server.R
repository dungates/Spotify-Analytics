...
server <- function(input, output, session) {
      ...
      output$similar_table <- renderDataTable({
            shiny::validate(need(spotify_df$df, message = FALSE))
      ...
      output$plot_2dpca <- renderHighchart({
            ...
            if(nrow(spotify_df$df)>0) {
                  ...
                  hchart(df_plot, "scatter", hcaes(PC1, PC2, group = grouped_by)) %>%
                        ...
                        hc_add_theme(hc_theme(chart = list(style = list(fontFamily = "Open Sans"))))
            }
      ...
      output$data_exp <- renderUI({
            validate(need(spotify_df$df, message = "Select an artist from the sidebar."))
            if(nrow(spotify_df$df)>0) {
                  tagList(
